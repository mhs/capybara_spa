...
=end
  ...
  config.before(:suite) do
    ...
    if Dir.exist?(output_path)
      puts <<-MESSAGE.gsub(/^\s*\|/, '')
        |
        |Using angular app found at #{output_path}
        |To rebuild the app delete this directory, then re-run the tests.
        |
      MESSAGE
    else
      puts <<-MESSAGE.gsub(/^\s*\|/, '')
        |
        |Angular app not found at #{output_path}
        |Building. This may take a minute...
        |
      MESSAGE

      build_static_angular_app.call
    end
  end
end
